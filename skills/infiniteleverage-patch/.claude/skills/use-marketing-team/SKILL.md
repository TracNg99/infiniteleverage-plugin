...
| Agent | File | Handles |
|---|---|---|
| **Writer** | `~/.claude/agents/writer.md` | Campaign strategy setup, blog posts, SEO content, copy, social media, translations |
...
|---|---|---|
| **Campaign setup (run first)** | Writer → `marketing-strategist` | "marketing strategy", "new campaign", "client brief", "scaffold marketing", "synthesize interview", "start marketing" |
...
```
[NEW CAMPAIGN]
Writer → marketing-strategist skill
  (reads client interview → writes context/strategy.md,
   context/content-process.md, context/content-calendar.md)
    ↓
Writer (drafts content from strategy.md angles)
    ↓
...
```
marketing-strategist        — campaign setup: interview → strategy.md + calendar (run before any writing)
